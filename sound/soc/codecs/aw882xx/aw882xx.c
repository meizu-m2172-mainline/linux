...
static int aw882xx_i2c_probe(struct i2c_client *i2c)
{
    ...
    if (IS_ERR(aw882xx->aw_pinctrl)) {
        ...
        aw_dev_info(aw882xx->dev,  "Cannot get pinctrl: %d, using GPIO fallback\n", ret);
        /* pinctrl is optional - continue with GPIO-only mode */
    } else {
        for (i = 0; i < ARRAY_SIZE(pctl_names); i++) {
            const char *n = pctl_names[i];
            ...
            aw_dev_dbg(aw882xx->dev, "found pin control %s\n", n);
