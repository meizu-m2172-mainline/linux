...
static int aw882xx_load_reg(struct aw882xx *aw882xx)
{
    ...
    return request_firmware_nowait(THIS_MODULE, 0,
        aw882xx_cfg_name[aw882xx->cfg_num],
    ...
    struct snd_ctl_elem_value *ucontrol)
{
    ...
    value = (reg_val >> mc->shift) & AW882XX_VOL_MASK;
...
static unsigned int aw882xx_component_read(struct snd_soc_component *component,
	unsigned int reg)
{
	struct aw882xx *aw882xx = snd_soc_component_get_drvdata(component);
	unsigned int value = 0;
...
static int aw882xx_component_write(struct snd_soc_component *component,
...
static const struct snd_soc_component_driver soc_component_dev_aw882xx = {
	.probe = aw882xx_probe,
	.remove = aw882xx_remove,
	.read = aw882xx_component_read,
	.write = aw882xx_component_write,
		...
		struct device_node *np)
{
	...
	unsigned int *sound_channel = &aw882xx->profile.channel;
	/* gpio */
	...
	ret = of_property_read_u32(np, "sound-channel", sound_channel);
	if (ret) {
		aw882xx->profile.channel = 0;
		dev_err(dev, "%s: sound-channel get failed,user default value!\n", __func__);
	} else {
		dev_info(dev, "%s: sound-channel = %d\n",
			__func__, aw882xx->profile.channel);
	}
	return 0;
}
...
int aw882xx_read_chipid(struct aw882xx *aw882xx)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		case AW882XX_ID:
			...
			pr_info("%s: aw882xx->chipid=0x%x\n",
				__func__, aw882xx->chipid);
...
static int aw882xx_i2c_probe(struct i2c_client *i2c)
{
	...
	/* aw882xx rst & int */
	if (np) {
		ret = aw882xx_parse_dt(aw882xx->dev, aw882xx, np);
		if (ret) {
			dev_err(aw882xx->dev,
	...
	if (gpio_is_valid(aw882xx->reset_gpio)) {
		...
		if (ret) {
			dev_err(aw882xx->dev, "%s: rst request failed\n",
				__func__);
	...
	if (gpio_is_valid(aw882xx->irq_gpio)) {
		...
		if (ret) {
			dev_err(aw882xx->dev, "%s: int request failed\n", __func__);
	...
	if (ret < 0) {
		dev_err(aw882xx->dev, "%s: aw882xx_read_chipid failed ret=%d\n",
			__func__, ret);
		goto err_id;
	}
	...
	dai = devm_kzalloc(aw882xx->dev, sizeof(aw882xx_dai), GFP_KERNEL);
	if (!dai)
	...
	// 		aw882xx_dai, ARRAY_SIZE(aw882xx_dai));
	ret = snd_soc_register_component(aw882xx->dev, &soc_component_dev_aw882xx,
			aw882xx_dai, ARRAY_SIZE(aw882xx_dai));
	if (ret < 0) {
		dev_err(aw882xx->dev, "%s failed to register aw882xx: %d\n",
			__func__, ret);
	...
	if (gpio_is_valid(aw882xx->irq_gpio) &&
		...
		ret = devm_request_threaded_irq(aw882xx->dev,
				...
				"aw882xx", aw882xx);
		if (ret != 0) {
			dev_err(aw882xx->dev, "failed to request IRQ %d: %d\n",
				gpio_to_irq(aw882xx->irq_gpio), ret);
			goto err_irq;
		}
	} else {
		dev_info(aw882xx->dev, "%s skipping IRQ registration\n", __func__);
		/* disable feature support if gpio was invalid */
		aw882xx->flags |= AW882XX_FLAG_SKIP_INTERRUPTS;
	}

	dev_set_drvdata(aw882xx->dev, aw882xx);
	ret = sysfs_create_group(&aw882xx->dev->kobj, &aw882xx_attribute_group);
	if (ret < 0) {
...
err_id:
    if (gpio_is_valid(aw882xx->irq_gpio))
        devm_gpiod_put(&i2c->dev, gpio_to_desc(aw882xx->irq_gpio));
err_irq_gpio_request:
    if (gpio_is_valid(aw882xx->reset_gpio))
        devm_gpiod_put(&i2c->dev, gpio_to_desc(aw882xx->reset_gpio));
...
static int __init aw882xx_i2c_init(void)
{
	...
	pr_info("%s: aw882xx driver added successfully\n", __func__);
	return ret;
}
