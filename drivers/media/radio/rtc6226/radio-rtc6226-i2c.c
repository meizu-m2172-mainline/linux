...
static int rtc6226_pinctrl_init(struct rtc6226_device *radio)
{
	...
	/* 使用正确的 devm_pinctrl_get 调用 */
	radio->fm_pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(radio->fm_pinctrl)) {
		/* 如果返回 -ENODEV，表示没有配置pinctrl，这是正常的 */
		if (PTR_ERR(radio->fm_pinctrl) == -ENODEV) {
			FMDERR("%s: target does not use pinctrl\n", __func__);
			radio->fm_pinctrl = NULL;
			return 0;
		}
		FMDERR("%s: failed to get pinctrl, error %ld\n", __func__,
		       PTR_ERR(radio->fm_pinctrl));
		return PTR_ERR(radio->fm_pinctrl);
	}

	/* 检查pinctrl是否为NULL（IS_ERR_OR_NULL的替代） */
	if (!radio->fm_pinctrl) {
		FMDERR("%s: pinctrl is NULL\n", __func__);
		return -ENODEV;
	}

	radio->gpio_state_active = pinctrl_lookup_state(radio->fm_pinctrl,
							"pmx_fm_active");
	if (IS_ERR(radio->gpio_state_active)) {
		FMDERR("%s: cannot get FM active state, error %ld\n", __func__,
		       PTR_ERR(radio->gpio_state_active));
		retval = PTR_ERR(radio->gpio_state_active);
		radio->gpio_state_active = NULL;
		/* 不立即返回，继续检查suspend状态 */
	} else {
		FMDERR("%s: found active state\n", __func__);
	}

	radio->gpio_state_suspend = pinctrl_lookup_state(radio->fm_pinctrl,
							 "pmx_fm_suspend");
	if (IS_ERR(radio->gpio_state_suspend)) {
		FMDERR("%s: cannot get FM suspend state, error %ld\n", __func__,
		       PTR_ERR(radio->gpio_state_suspend));
		/* 如果之前active状态也失败，返回错误 */
		if (retval != 0)
			retval = PTR_ERR(radio->gpio_state_suspend);
		radio->gpio_state_suspend = NULL;
	} else {
		FMDERR("%s: found suspend state\n", __func__);
	}

	/* 如果两个状态都找不到，清理pinctrl并返回错误 */
	if (!radio->gpio_state_active && !radio->gpio_state_suspend) {
		devm_pinctrl_put(radio->fm_pinctrl);
		radio->fm_pinctrl = NULL;
